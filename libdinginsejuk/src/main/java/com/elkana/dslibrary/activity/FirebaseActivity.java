...
//        DataUtil.cleanTransactionData();
    ...
    public void syncUserInformation(){
        ...
        BasicInfo basicInfo = realm.where(BasicInfo.class)
                .equalTo("uid", mAuth.getCurrentUser().getUid())
                .findFirst();

        if (basicInfo == null) {
            database.getReference("users/" + mAuth.getCurrentUser().getUid()).child("basicInfo")
...
//                .count() < 1) {
            database.getReference("users/" + mAuth.getCurrentUser().getUid()).child("address")
                    .addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            if (!dataSnapshot.exists()) {
